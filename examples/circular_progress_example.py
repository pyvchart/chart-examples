...
c = (
    CircularProgress()
    .set_data(data=[opts.BaseDataOpts(id_="id0", values=TEST_CIRCLE_PROGRESS_DATA)])
    ...
    .set_global_options(
        ...
        axes_opts=[
            opts.AxesLinearOpts(
                is_visible=False, base_axes_opts=opts.BaseAxesOpts(orient="angle")
            ),
            opts.AxesBandOpts(
                base_axes_opts=opts.BaseAxesOpts(is_visible=False, orient="radius")
            ),
        ],
...
c.render("circular_progress_example.html")
