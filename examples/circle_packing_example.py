...
c = (
    CirclePacking()
    .set_data(data=[opts.BaseDataOpts(id_="data", values=TEST_CIRCLE_PACKING_DATA)])
    ...
    .set_global_options(
        ...
        tooltip_opts=opts.TooltipOpts(
            mark_opts=opts.TooltipCustomOpts(
                title_value=JsCode(
                    "val => { return val?.datum?.map(data => data.name).join(' / '); }",
                )
            )
        ),
    )
)
c.render("circle_packing_example.html")
